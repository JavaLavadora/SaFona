"""Process AI-generated tile images into auto-tile strips for cave tilesets.

Variant of process_ai_tiles.py with a cool cave palette instead of
warm Mediterranean stone. Remaps to dark blue-grey stone with
moss/damp green surface tints.

Usage:
    python tools/process_ai_tiles_cave.py [input_image] [output_path]

Defaults:
    input:  assets/ai_sources/tileset_world1_cave/image.png
    output: assets/tilesets/world1_cave/tileset.png
"""
...
# Cool cave palette (dark blue-grey stone).
CAVE_DARK = np.array([80, 85, 95], dtype=np.float32)
CAVE_LIGHT = np.array([160, 165, 175], dtype=np.float32)

# Moss/damp green tint for cave surfaces.
MOSS_DARK = np.array([45, 75, 55], dtype=np.float32)
MOSS_LIGHT = np.array([80, 115, 75], dtype=np.float32)
...
def cave_correct(tile_arr: np.ndarray) -> np.ndarray:
    """Remap pixels onto cool cave stone palette."""
    out = tile_arr.astype(np.float32)
    lum_norm = _normalize(_luminance(out))

    for c in range(3):
        out[:, :, c] = CAVE_DARK[c] + lum_norm * (CAVE_LIGHT[c] - CAVE_DARK[c])
...
def cave_correct_moss(tile_arr: np.ndarray) -> np.ndarray:
    """Moss/damp green top, cave stone bottom."""
    h = tile_arr.shape[0]
    moss_end = int(h * 0.4)

    stone = cave_correct(tile_arr[moss_end:])

    moss = tile_arr[:moss_end].astype(np.float32)
    lum_norm = _normalize(_luminance(moss))
    moss_out = np.zeros_like(moss)
    for c in range(3):
        moss_out[:, :, c] = MOSS_DARK[c] + lum_norm * (MOSS_LIGHT[c] - MOSS_DARK[c])

    return np.vstack([np.clip(moss_out, 0, 255).astype(np.uint8), stone])
...
def build_autotile_strip(
    ...
    bases = {
        "top_moss": top_moss.convert("RGBA"),
...
def main() -> None:
    input_path = (
        ...
        else PROJECT_ROOT / "assets" / "ai_sources" / "tileset_world1_cave" / "image.png"
    )
    output_path = (
        Path(sys.argv[2]) if len(sys.argv) > 2
        else PROJECT_ROOT / "assets" / "tilesets" / "world1_cave" / "tileset.png"
    )
    ...
    corrected = [
        cave_correct_moss(tiles[0]),
        cave_correct(tiles[1]),
        np.clip(cave_correct(tiles[2]).astype(np.float32) * 0.7, 0, 255).astype(np.uint8),
        cave_correct(tiles[3]),
    ]
