...
def process_character(
    ...
    Args:
        ...
        only: When set, restrict which animation is *written to disk* to the
            ...
            Steps 1-2 still attempt to load and measure every entry, but a
            missing source (`source_path.exists()` false, e.g. a sibling
            animation not yet generated by an incremental img2vid run) is
            skipped from `animations` entirely (Step 1) and therefore from
            `global_max_h` (Step 2) -- so ``base_scale`` matches a full run
            ONLY once every entry's source file actually exists on disk. On
            an incremental run with some sources still missing, base_scale
            is provisional and can shift once a taller sibling is generated
            later; re-run without ``--only`` after every animation exists to
            lock in the final scale. Only Step 3's save/GIF is skipped for
            entries other than ``only``.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Only write this one animation's sheet + GIF (e.g. walk). "
             "Every PRESENT source is still loaded/measured for the shared "
             "base scale, but a missing sibling source is skipped from that "
             "computation -- scale is only final once every animation's "
             "source exists. Requires a single config path.",
    )
    return parser
